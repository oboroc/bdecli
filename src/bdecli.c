 ...
 * 
 * Copyright (C) 2010-2018 Adrian Oboroc
 *
...
/* free all memory allocated for linked list */
void bde_cfg_free(bde_entry_t *list) {
  bde_entry_t *item;
...
int bde_cfg_add_entry(bde_entry_t *list, char *szFQNPath, char *szName, char *szValue) {
  ...
  if (BDE_CONTAINER != list->entry_type) {
    /* sanitize list */
  ...
  while (0 != tmp[i]) {
    if (('\\' == tmp[i]) || (0 == tmp[i + 1])) {
      if (i > MAX_BUFFER) {
        /* sanitize possible buffer overflow */
      ...
      if (!co) {
        /* container not found, we should create a new one */
  ...
  while ((current) && (current->container == co)) {
    if ((BDE_VARIABLE == current->entry_type) &&
      (0 == strcmp(current->name, szName))) {
      /* found variable, compare it's value to new value */
      if (0 == strcmp(current->value, szValue))
        return 0;
      else {
...
int main(int argc, char *argv[]) {
  ...
  if (1 == argc) {
    printf_s("bdecli %s (c) 2010-2018 Adrian Oboroc https://github.com/oboroc/bdecli/ (%s)\n\n", BDECLI_VER, __DATE__);
    printf_s(
